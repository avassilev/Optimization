...
\begin{itemize}\itemsep1em
\item Notice that a solution to the Hamiltonian maximization problem 
...
\item ...or directly through the resulting vector (\textcolor{red}{the red arrows}).
...
\item They are also called \emph{integral }curves and are drawn with arrows indicating the direction in which a particular curve is traversed over time.
\end{itemize}
\column{.4\textwidth}
\begin{center}
\input{SS2input.tex}
